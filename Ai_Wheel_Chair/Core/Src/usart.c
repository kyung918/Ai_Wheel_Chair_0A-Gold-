...
{  
	switch((uint32_t)huart->Instance){
		...
		case (uint32_t)USART1:
			...
			HAL_UART_Receive_IT(&huart1, &buff, 1); // 1����Ʈ ����
...
/*			__HAL_UART_CLEAR_OREFLAG(&huart2);
			HAL_UART_Receive_IT(&huart2, &buff, 1); // 1����Ʈ ����
		...
		case (uint32_t)USART3:
			...
			HAL_UART_Receive_IT(&huart3, &buff, 1); // 1����Ʈ ����
		...
		case (uint32_t)UART4:
			...
			HAL_UART_Receive_IT(&huart4, &buff, 1); // 1����Ʈ ����
		...
		case (uint32_t)UART5:
			...
			HAL_UART_Receive_IT(&huart5, &buff, 1); // 1����Ʈ ����
		...
		case (uint32_t)USART6:
			...
			HAL_UART_Receive_IT(&huart6, &buff, 1); // 1����Ʈ ����
   ...
   COMM_BUF      *ptr;
    ...
    while(1){
     	...
     	switch (port) {
		  ...
		  if (ptr->Ctr >= UART_MAXQ-1) {   // 버퍼 ?��버플로우 처리
				CommBufClear(port);
				break;
			}
		  break;
		  case COM3:
/*
			if(remotetype.bRemoteType==false){
			...
			//첫자�???????????????? �?????????????????��
				...
				//?��?�� ?��?��?�� 받는�????????????????
				if((ptr->Buf[0]== 0x0f)&&(ptr->Ctr == 25)) { // ETX�???????????????? �???????????????? 마�?막에 ?��?��
					ptr->RxCompletion = 1;   // ?��?��?�� ?��?�� ?���????????????????

					update_channels(&ptr->Buf);
					//printf("%c\n",ptr->Buf);
					...
					//All Brake / Tq off / 주행 ?��?�� S/W
					// 320      / 1000   / 2000
					if(channels[6] == 1000){
						r_cPid = 0xae;
					}
					else if(channels[6] <= 800){
						r_cPid = 0xaf;
					}
					else if(channels[6] >= 1100){
						r_cPid = 0xcf;
					}
...
void update_channels(uint8_t *buf) {
    ...
    uint8_t i;
		...
		//channels[1] = ?��?��(?��?��) / channels[2] = 각속?��
		//?��?�� 리모�??????????????? �??????????????? 320 ~ 999 ~ 1694
		//?��?�� =  (리모콘값 / 10) - 100(중간�???????????????) *(float)(?��?��?�� ?���????????????????�� / ?��?���??????????????? ?���????????????????��) * 0.1(�???????????????경해볼것)
